...
export interface JunkBondOpportunity {
  ticker: string;
  title: string;
  yesPrice: number;
  category: string;
...
export interface JunkBondConfig {
  minPrice: number;
  maxPrice: number;
  minVolume: number;
...
export class JunkBondStrategy {
  private config: JunkBondConfig;
  
  constructor(config: Partial<JunkBondConfig> = {}) {
    this.config = {
      minPrice: 0.70,
      maxPrice: 0.95,
      minVolume: 100,
   ...
   */
  findOpportunities(markets: any[]): JunkBondOpportunity[] {
    const opportunities: JunkBondOpportunity[] = [];
    ...
    for (const market of markets) {
      ...
      const isTargetCategory = this.config.targetCategories.some(
        tc => category.includes(tc)
      );
      if (!isTargetCategory) continue;
      
      // Skip if price outside junk bond range
      if (yesPrice < this.config.minPrice || yesPrice > this.config.maxPrice) continue;
  ...
  calculatePositionSize(
    ...
    const convictionMultipliers: Record<string, number> = {
      'VERY_HIGH': 1.0,
      'HIGH': 0.75,
      'MEDIUM': 0.5,
      'LOW': 0.25
   ...
   */
  private calculateConviction(price: number, category: string): 'LOW' | 'MEDIUM' | 'HIGH' | 'VERY_HIGH' {
    // Price-based scoring
    if (price >= 0.90) return 'VERY_HIGH';
    if (price >= 0.85) return 'HIGH';
    if (price >= 0.80) return 'MEDIUM';
    if (price >= 0.70) return 'LOW';
    return 'LOW';
  }
   ...
   */
  private estimateDaysToResolution(market: any): number {
   ...
   */
  shouldExit(currentPrice: number, entryPrice: number): boolean {
    // Exit if price drops below 60% (unusual event happened)
    if (currentPrice < 0.60) return true;
    
    // Exit if price spikes above 98% (take profit)
    if (currentPrice > 0.98) return true;
