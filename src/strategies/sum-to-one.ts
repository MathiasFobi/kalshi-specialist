...
export interface SumToOneOpportunity {
  ticker: string;
  title: string;
  yesPrice: number;
  noPrice: number;
...
export interface SumToOneConfig {
  minSum: number;
  maxSum: number;
  minVolume: number;
}
...
export class SumToOneStrategy {
  private config: SumToOneConfig;
  
  constructor(config: Partial<SumToOneConfig> = {}) {
    this.config = {
      minSum: 0.50,
      maxSum: 0.95,
      minVolume: 100,
   ...
   */
  findOpportunities(markets: any[]): SumToOneOpportunity[] {
    const opportunities: SumToOneOpportunity[] = [];
    ...
    for (const market of markets) {
      ...
      // Skip if no volume
      if ((market.volume || 0) < this.config.minVolume) continue;
